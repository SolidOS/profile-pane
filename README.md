...
Debug logging is configured through [src/utils/debug.ts](src/utils/debug.ts).

Default behavior:

- Dev entrypoint in [dev/index.ts](dev/index.ts) defaults to `trace`, so all debug logs are enabled during local development.
- Production pane entrypoint in [src/index.ts](src/index.ts) defaults to `error`, so only errors are logged unless you override the level.
...
Runtime override options for both dev and production:

1. URL parameter:

```text
http://localhost:8080/?debug=trace
```

2. Browser local storage:

```js
localStorage.setItem('profile-pane:debug', 'trace')
```

3. Global browser flag before startup:

```js
window.__PROFILE_PANE_DEBUG__ = 'trace'
```

Priority order is:

1. `?debug=` URL parameter
2. `localStorage['profile-pane:debug']`
3. `window.__PROFILE_PANE_DEBUG__`
4. code default from the active entrypoint

To turn logging off completely:
...
```js
localStorage.setItem('profile-pane:debug', 'silent')
```

To clear a persistent local override:

```js
localStorage.removeItem('profile-pane:debug')
```

When using the utility in code:

```ts
import { createDebugLogger, log, warn, error, trace } from './src/utils/debug'

trace('Detailed render state', state)
log('Pane mounted')
warn('Profile load fallback path used', error)
error('Profile save failed', error)

const debug = createDebugLogger('heading')
debug.warn('Profile image upload failed', error)
```

Use [src/utils/debug.ts](src/utils/debug.ts) instead of direct `console.log`, `console.warn`, `console.error`, and related console calls.

Only logs routed through [src/utils/debug.ts](src/utils/debug.ts) are controlled by these levels.
