import { graph, LiveStore, NamedNode, Node, st } from 'rdflib'
...
export function ensureStandardMutationPrefixes(store: LiveStore | undefined): void {
  ...
  const targets: Array<PrefixCapable | undefined> = [
    store as PrefixCapable,
    updater,
    updater?.store as PrefixCapable | undefined
  ]
...
export async function putResourceWithStatements(
  ...
  const serializationStore = graph() as LiveStore
...
export async function runUpdateTransport(
  ...
  if (doc) {
    const fetcherAny = getStoreFetcher(store) as any
    const editableState = typeof (updater as any)?.editable === 'function'
      ? (updater as any).editable(doc.value, store)
      : undefined
    if (editableState === undefined && typeof fetcherAny?.refresh === 'function') {
      try {
        await fetcherAny.refresh(doc)
      } catch {
