...
/*
  describe("without friends", () => {
    ...
    it("renders an empty friend list", () => {
      expect(element.textContent.trim()).toBe("Friends");
    });
  });

  describe("with more skills in separate document", () => {
    beforeAll(async () => {
      store.removeDocument(doc);
      const turtle = `
      @prefix : <#>.
      ...
      :me rdfs:seeAlso <./cv.ttl>;
      .
  `;
      parse(turtle, store, doc.uri);
      fetchMock.mockOnceIf(
        "https://janedoe.example/profile/cv.ttl",
        `
          @prefix jane: </profile/card#>.
          @prefix foaf: <http://xmlns.com/foaf/0.1/> .
          @prefix vcard: <http://www.w3.org/2006/vcard/ns#> .
          jane:me schema:skills [
            schema:name "driving very fast";
          ], [
            schema:name "testing software";
          ];
      .
      `,
        {
